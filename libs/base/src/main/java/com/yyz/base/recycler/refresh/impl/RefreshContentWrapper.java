...
import com.yyz.base.recycler.refresh.PagerAdapterWrapper;
...
public class RefreshContentWrapper implements RefreshContent {
    ...
    protected void wrapperCoordinatorLayout(CoordinatorLayout layout, final RefreshLayoutImpl refreshLayout) {
    ...
    protected class Api23ViewScrollComponent implements View.OnScrollChangeListener {
        ...
        public void onScrollChange(View v, int scrollX, int scrollY, int oldScrollX, int oldScrollY) {
            ...
            RefreshLayoutImpl layout = kernel.getRefreshLayout();
            ...
            if (scrollY <= 0 && oldScrollY > 0 && mMotionEvent == null && lastTime - lastTimeOld > 1000 && overScroll && layout.isEnableRefresh()) {
                ...
                } else if (overScroll && lastTime - lastTimeOld > 1000 && !ScrollBoundaryUtil.canScrollDown(v)) {
                    final int velocity = (lastOldScrollY - oldScrollY) * 16000 / (int) ((lastTime - lastTimeOld) / 1000f);
    ...
    protected class AbsListViewScrollComponent implements AbsListView.OnScrollListener {
        ...
        int lastScrollDy;
        RefreshKernel kernel;
        SparseArray<ItemRecord> recordSp = new SparseArray<>(0);
        ...
        protected int getScrollY(AbsListView view, int firstVisibleItem) {
            ...
            if (null != firstView) {
                ...
                for (int i = 0; i < firstVisibleItem; i++) {
                    ItemRecord iRecord = recordSp.get(i);
                    if (iRecord != null) {
                        height += iRecord.height;
                        lastHeight = iRecord.height;
                    } else {
                        height += lastHeight;
                    }
                }
                ItemRecord iRecord = recordSp.get(firstVisibleItem);
                if (null == iRecord) {
                    iRecord = new ItemRecord();
                }
                return height - iRecord.top;
            }
            return 0;
        }
    ...
    protected class RecyclerViewScrollComponent extends RecyclerView.OnScrollListener {
        ...
        public void onScrolled(RecyclerView recyclerView, int dx, int dy) {
            if (mMotionEvent == null) {
                final RefreshLayoutImpl layout = kernel.getRefreshLayout();
                ...
                } else if (dy > 0 && layout.isEnableLoadMore() && !ScrollBoundaryUtil.canScrollDown(recyclerView)) {
                    if (layout.getState() == RefreshState.None
                            && layout.isEnableAutoLoadMore()
                            && !layout.isLoadMoreFinished()
                            && !layout.isEnablePureScrollMode()) {
